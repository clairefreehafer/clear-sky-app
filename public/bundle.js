...
var locationKeysUrl = 'http://www.cleardarksky.com/t/chart_prop00.txt';

var keysArr = [];
var locationArr = [];
var nameArr = [];

var locationKeysArr = void 0;
var locationData = {};

var getLocationArray = function getLocationArray() {
	return axios.get('../d3-sandbox/obs_data.tsv').then(function (body) {
...
var getLocationData = function getLocationData(key) {
	var locationDataUrl = 'http://cleardarksky.com/txtc/' + key + 'csp.txt';
	...
	return axios.get('../d3-sandbox/obs_data.tsv').then(function (body) {
		...
		var dataArr = body.match(/"(.*)",\s(\d*),\t(\d*),\t(\d*),\t(\d*),\t(\d*),\t(\d*),\t/g); // array of each hour's data
		var darknessArr = body.match(/"(.*)",\s([-\.\d]*),\s([-\.\d]*),\s([-\.\d]*)(\))/g);

		dataArr.forEach(function (data) {
			var hourlyData = data.replace(/\s/g, '').split(','); // one array with each column its own element
			var hour = hourlyData[0].split(':')[0]; // YYYY-MM-DDHH
		...
		darknessArr.forEach(function (data, index) {
			var darknessData = data.replace(/\s|\)/g, '').split(',');
			var time = darknessData[0].slice(11, -1).split(':');
...
var cloudColors = ['#003F7F', '#135393', '#2767A7', '#4F8FCF', '#63A3E3', '#77B7F7', '#9ADADA', '#AEEEEE', '#C2C2C2', '#EAEAEA', '#FBFBFB'];
...
// data signifiers
var arc = d3.arc().innerRadius(innerRadius).outerRadius(function (d) {
...
var svg = d3.select('body').append('svg').attr('width', width) // cuts off graph
.attr('height', height) // cuts off graph
.attr('fill', 'white').append('g').attr('transform', 'translate(' + width / 2 + ',' + height / 2 + ')'); // position
...
d3.csv('../d3-sandbox/obs_data.csv', function (error, data) {
	...
	.attr('class', 'solidArc').attr('d', arc).attr('stroke', 'none')
	// data hover
